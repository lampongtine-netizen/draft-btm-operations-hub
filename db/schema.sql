...
drop policy if exists btm_staff_read_communication_attachments on storage.objects;
create policy btm_staff_read_communication_attachments
on storage.objects for select to authenticated
using (bucket_id = 'communication-attachments' and (select private.is_btm_staff()));

drop policy if exists btm_staff_upload_communication_attachments on storage.objects;
create policy btm_staff_upload_communication_attachments
on storage.objects for insert to authenticated
with check (bucket_id = 'communication-attachments' and (select private.is_btm_staff()));
